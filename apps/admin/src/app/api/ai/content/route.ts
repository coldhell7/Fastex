...
function buildPrompt(title: string, body: string, keywords: string) {
  const template = getContentPrompt();
  return template
    .replace(/\{title\}/g, `Existing title:\n${title}`)
    .replace(/\{body\}/g, body ? `Existing body:\n${body.slice(0, 1000)}` : "")
...
export async function POST(req: Request) {
  try {
    ...
    const title = jsonBody.title?.trim() || "نوشته بدون عنوان";
    const existingBody = jsonBody.body?.trim() || "";
    const keywords = jsonBody.keywords?.trim() || "";

    const redis = getRedis();
    const cacheKey = `ai:content:${createHash("sha256").update(`${title}|${keywords}`).digest("hex")}`;
    ...
    if (redis) {
      ...
      const rateKey = `ratelimit:ai:content:${ip}`;
